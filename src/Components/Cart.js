...
const Cart = () => {
  ...
  const addCartHandler = (item) => {
    ...
    toast.dark('Product added sucsessfull')
  }
          ...
          {mainCart.map(item => (
            <div key={item.id} className='col'>
              <div className='card shadow-sm p-3' style={{
                ...
                <img
                  src={item.image}
                  alt={item.title} className="bg-placeholder card-image-top"
                  width='100%'
                  height='350px'
                />
                ...
                <button onClick={() => addCartHandler(item)} className='mt-3 btn btn-outline-primary'>Add Card</button>
              </div>
            </div>
          ))}
        </div>
